...
export default function NewAreaCategoryPage() {
    const router = useRouter();
    const [isSubmitting, setIsSubmitting] = useState(false);

    const handleSubmit = async (data: any) => {
        setIsSubmitting(true);
        const result = await createAuditMasterItem("area-categories", data);
        setIsSubmitting(false);

        if (result.success) {
            toast.success("Audit Area Type created successfully");

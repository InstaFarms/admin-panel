...
export default function EditChecklistCategoryPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const result = await getAuditMasterData("checklist-categories", { perPage: 1000 });
            if (result.success) {
    ...
    const handleUpdate = async (data: any) => {
        ...
        const result = await updateAuditMasterItem("checklist-categories", id, data);
        setIsSubmitting(false);

        if (result.success) {
            toast.success("Checklist Category updated successfully");
            router.push("/admin/audit-master/checklist-categories");
        } else {

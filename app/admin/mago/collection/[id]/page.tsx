...
export default async function Page({ params }: ServerPageProps) {
  ...
  const rawData = idString ? await getCollectionWithProperties(idString, "mago") : null;
  if (!rawData) notFound();

  const breadcrumbs = getCollectionBreadcrumbs("mago");
    ...
    <div className="flex w-full flex-col">
      <Card className="w-full bg-white dark:bg-gray-800">
        ...
        <div className="mx-auto flex w-full max-w-5xl flex-col gap-5 overflow-x-auto rounded-xl bg-slate-100 p-5 dark:bg-gray-900">
          ...
          <CollectionEditor data={rawData} adminScope="mago" />
        </div>
      </Card>
    </div>
  );

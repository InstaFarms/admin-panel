...
export default async function Page({ searchParams }: ServerPageProps) {
  ...
  const result = await getCollectionsList(limit, pageNumber, searchValue, searchBy, "mago");
    ...
    <div className={styles.page}>
      <div className={styles.pageHeader}>
        ...
        <div className={styles.toolbar}>
          ...
          <Link href="/admin/mago/collection/create" className={styles.createButton}>
            <Plus size={16} />
            New
          </Link>
        </div>
              ...
              {data.length > 0 ? (
                data.map(({ id, name, slug, isActive, weight, logo }: any, i: number) => (
                  <tr key={id} className={styles.tableRow}>
                    ...
                    <td className={styles.tableCell}>
                      <Link href={`/admin/mago/collection/${id}`} className={styles.stateLink}>
                        {name}
                      </Link>
                    </td>
                    ...
                    <td className={styles.tableCell}>
                      <Link href={`/admin/mago/collection/${id}`} className={styles.actionButton} title="Edit">
                        <Pencil size={16} />
                      </Link>
                    </td>
                  </tr>

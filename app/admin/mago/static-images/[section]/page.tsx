...
import {
  VALID_SECTIONS, getSectionLabel,
  type StaticImageSection, staticImagesSectionBreadcrumbs,
...
export default async function Page({
  ...
  try {
    data = await getStaticImagesBySection(section as StaticImageSection, false, SCOPE);
      ...
      <Card className="w-full bg-white dark:bg-gray-800">
        <div className="flex w-full flex-col gap-4">
          <h5 className="text-xl font-bold tracking-tight text-gray-900 dark:text-white">
            Edit {sectionLabel} (Mago)
          </h5>
          <PageBreadcrumb items={staticImagesSectionBreadcrumbs(SCOPE, sectionLabel)} className="pb-3" />
        </div>
        <StaticImageEditor data={data} section={section as StaticImageSection} adminScope={SCOPE} />
      </Card>
    </div>
  );
}

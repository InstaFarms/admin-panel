...
export default async function Page({ params }: ServerPageProps) {
  ...
  const data = await getCancellationPlanById(idString, "mago");

  if (!data) {
    throw new Error(CANCELLATION_PLANS_ERRORS.notFound);
  }

  const breadcrumbs = getCancellationPlanBrandBreadcrumbs("mago");
    ...
    <div className="flex w-full flex-col">
      <Card className="w-full bg-white dark:bg-gray-800">
        ...
        <div className="w-full max-w-4xl mx-auto rounded-xl bg-slate-100 p-4 sm:p-6 lg:p-8 dark:bg-gray-900">
          ...
          <CancellationPlanEditor data={data} adminScope="mago" />
        </div>
      </Card>
    </div>
  );

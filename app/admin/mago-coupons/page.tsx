...
import { SEARCH_KEYS, TABLE_COLUMNS, COUPON_STATUS_CONFIG, MAGO_COUPON_BREADCRUMBS } from "@/constants/coupons";

import { getCouponStatusKey, formatDate, formatDiscount } from "@/lib/couponUtils";

export const dynamic = "force-dynamic";

export default async function MagoCouponsPage({ searchParams }: ServerPageProps) {
    const params = await searchParams;
    const { offset } = parseLimitOffset(params);
    const filterParams = parseFilterParams(params);

    const brandName = CUSTOMER_BRANDS.MAGO;
            ...
            <Card className="w-full bg-white dark:bg-gray-800">
                <div className="flex w-full flex-col gap-2">
                    ...
                    <div className="flex w-full flex-col gap-3 pb-3 sm:flex-row sm:items-center sm:justify-between">
                        <PageBreadcrumb items={MAGO_COUPON_BREADCRUMBS.list} className="w-full shrink-0 sm:w-auto" />
                        <div className="flex w-full shrink-0 flex-row items-center justify-end gap-3 sm:w-auto">
                            ...
                            <CouponFilters />
                            <Link href="/admin/mago-coupons/create" className="shrink-0">
                                <Button>New</Button>
                            </Link>
                        </div>
                    </div>
                                    ...
                                    data.map((coupon, index) => {
                                        ...
                                        return (
                                            <TableRow key={coupon.id} className="bg-white dark:border-gray-700 dark:bg-gray-800">
                                                ...
                                                <TableCell>
                                                    <Link href={`/admin/mago-coupons/${coupon.id}`} title="Edit">

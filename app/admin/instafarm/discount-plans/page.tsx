...
import { getDiscountPlans } from "@/actions/discountPlanActions";
import { getEmptyListMessage } from "@/constants/ui";
import { DISCOUNT_PLANS_SEARCH_KEYS } from "@/constants/discount-plans";
import { DISCOUNT_PLANS_LIST_BASE_PATH } from "@/constants/planBrandScope";
...
const LIST = DISCOUNT_PLANS_LIST_BASE_PATH[SCOPE];
...
export default async function Page({ searchParams }: ServerPageProps) {
  ...
  const { data, total } = await getDiscountPlans(limit, offset, searchValue, searchBy, SCOPE);

  const hasNextPage = offset + data.length < total;
  const emptyMessage = getEmptyListMessage("discount plans", Boolean(searchValue));

  return (
    <div className={styles.page}>
      <div className={styles.pageHeader}>
        <div>
          <h1 className={styles.pageTitle}>Discount Plans</h1>
          <div className={styles.breadcrumbs}>
            <span>Dashboard</span>
            <ChevronRight size={13} />
            <span className={styles.breadcrumbsCurrent}>Discount Plans</span>
          </div>
        </div>
        <div className={styles.toolbar}>
          <div className={styles.searchBarWrap}>
            <Suspense fallback={null}>
              <Searchbar
                searchKeys={[...DISCOUNT_PLANS_SEARCH_KEYS]}

...
export default async function Page({ params }: ServerPageProps) {
  ...
  const data = await getDiscountPlanById(idString, "instafarms");

  if (!data) {
    throw new Error(DISCOUNT_PLANS_ERRORS.notFound);
  }

  const breadcrumbs = getDiscountPlanBrandBreadcrumbs("instafarms");
    ...
    <div className="flex w-full flex-col">
      <Card className="w-full bg-white dark:bg-gray-800">
        ...
        <div className="w-full max-w-4xl mx-auto rounded-xl bg-slate-100 p-4 sm:p-6 lg:p-8 dark:bg-gray-900">
          ...
          <DiscountPlanEditor data={data} adminScope="instafarms" />
        </div>
      </Card>
    </div>
  );

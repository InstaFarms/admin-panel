...
import { getEventEnquiries } from "@/actions/requestActions";

const TITLE = "Event Enquiries";
const BREADCRUMBS = [
  { href: "/", label: "Home" },
...
export default async function Page({ searchParams }: ServerPageProps) {
  ...
  try {
    data = await getEventEnquiries(searchParams);
  } catch (err) {
    console.error("Error fetching event enquiries:", err);
    error = err instanceof Error ? err.message : "Failed to load event enquiries.";
  }
            ...
            <Table className="min-w-full">
              <TableHead>
                <TableRow>
                  ...
                  <TableHeadCell className="whitespace-nowrap">Event Date</TableHeadCell>
                ...
                {!error && data.length > 0 ? (
                  data.map((enquiry, index) => (
                    <TableRow className="bg-white dark:border-gray-700 dark:bg-gray-800" key={enquiry.id}>
                      ...
                      <TableCell>
                        {formatAdminDate(enquiry.eventDate, "-")}
                      </TableCell>
                  ...
                  <TableRow>
                    <TableCell colSpan={9} className="py-4 text-center text-gray-500 dark:text-gray-400">
                      {error ?? "No event enquiries found."}
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>

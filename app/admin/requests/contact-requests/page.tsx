...
import { getContactRequests } from "@/actions/requestActions";

const TITLE = "Contact Requests";
const BREADCRUMBS = [
  { href: "/", label: "Home" },
  { href: "/admin", label: "Admin" },
  { href: "#", label: "Contact Requests" },
];
...
export default async function Page({ searchParams }: ServerPageProps) {
  ...
  try {
    data = await getContactRequests(searchParams);
  } catch (err) {
    console.error("Error fetching contact requests:", err);
    error = err instanceof Error ? err.message : "Failed to load contact requests.";
  }
            ...
            <Table className="min-w-full">
              <TableHead>
                <TableRow>
                  ...
                  <TableHeadCell className="whitespace-nowrap">Type</TableHeadCell>

...
export default async function EditInstafarmsProposalPage({ params }: ServerPageProps) {
    const { id } = await params;
    const idStr = typeof id === "string" ? id : id?.[0];
    if (!idStr) notFound();

    const proposal = await getProposalById(idStr, { brandName: CUSTOMER_BRANDS.INSTAFARMS });
    ...
    return (
        <div className="p-4">
            <div className="mb-4">
                <h1 className="text-2xl font-bold dark:text-white">Edit Instafarms Proposal</h1>

...
import { createOwnerWithProperties, updateOwnerProperties } from "@/actions/ownerActions";
...
export default function UserEditor(props: UserEditorProps) {
  ...
  const formId = props.formId ?? "owner-editor-form";
  ...
  const runPromise = (promise: Promise<string>, loadingLabel: string) => {
    ...
    toast.promise(guardedPromise, {
      ...
      success: (data) => {
        if (!props.data) {
          router.push("/admin/users/owners");
        } else {
          router.refresh();
        }
        return data;
  ...
  const handleSubmit = () => {
        ...
        return;
          ...
          if (props.data) {
            ...
            promise = parseServerActionResult(
              createOwnerWithProperties(ownerData, selectedPropertyIds)
            );
          }

          runPromise(promise, "Saving Owner data...");
        });
      };
      void run();
      return;
    ...
    startTransition(() => {
      ...
      if (props.data && mode === "properties") {
        promise = parseServerActionResult(
          updateOwnerProperties(props.data.id, selectedPropertyIds)
        );
      } else {
        if (!validateAll()) {
          releaseSubmitLock();

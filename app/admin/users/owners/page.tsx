/**
 * Owners list page: paginated table with search (Name, Email).
...
import { USERS_BREADCRUMBS, USERS_SEARCH_KEYS, HiPencil, HiCurrencyDollar } from "@/constants/users";
...
export default async function Page({ searchParams }: ServerPageProps) {
  ...
  const emptyMessage = getEmptyListMessage("owners", hasSearch);
      ...
      <Card className="w-full bg-white dark:bg-gray-800">
        ...
        <div className="flex w-full flex-col gap-2">
          ...
          <div className="flex w-full flex-col gap-3 pb-3 sm:flex-row sm:items-center sm:justify-between">
            <PageBreadcrumb items={USERS_BREADCRUMBS.owners.list} className="w-full shrink-0 sm:w-auto" />
            <div className="flex w-full shrink-0 flex-row items-center justify-end gap-3 sm:w-auto sm:justify-end">
              ...
              <Link href="/admin/users/owners/create" className="cursor-pointer shrink-0">
                <Button>New</Button>
              </Link>
            </div>
          </div>
                  ...
                  data.map((user, index) => (
                    ...
                    >
                      ...
                      <TableCell className="min-w-0 font-medium whitespace-nowrap text-gray-900 dark:text-white">
                        <Link
                          href={`/admin/users/owners/${user.id}`}
                      ...
                      <TableCell>
                        <SettlementToggle
                          ownerId={user.id}
                          initialEnabled={user.settlementEnabled ?? true}
                        />
                      </TableCell>
                      <TableCell>
                        <div className="flex flex-row items-center gap-3">
                          <Link
                            href={`/admin/users/owners/${user.id}/wallet`}
                            className="w-fit"
                            title="Wallet"
                          >
                            <div className="rounded-md bg-green-600 p-1 transition-colors hover:bg-green-700">

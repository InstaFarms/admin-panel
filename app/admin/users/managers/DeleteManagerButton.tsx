...
import { checkManagerRoleExists } from "@/actions/managerActions";
...
export default function DeleteManagerButton({ id }: { id: string }) {
  ...
  const handleSubmit = () => {
    startTransition(() => {
      const promise = (async () => {
        const managerRole = await checkManagerRoleExists(id);
        if (managerRole.error) {
          throw new Error(managerRole.error);
        }

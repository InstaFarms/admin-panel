...
import { createCaretakerWithProperties, updateCaretakerProperties } from "@/actions/caretakerActions";
...
interface CaretakerEditorProps {
  data?: User;
...
export default function CaretakerEditor(props: CaretakerEditorProps) {
  const [, startTransition] = useTransition();
  const router = useRouter();
  const submitLockRef = useRef(false);
  const formId = props.formId ?? "caretaker-editor-form";
  ...
  const runPromise = (promise: Promise<string>, loadingLabel: string) => {
    ...
    toast.promise(guardedPromise, {
      ...
      success: (data) => {
        if (!props.data) {
          router.push("/admin/users/caretakers");
        } else {
          router.refresh();
        }
        return data;
  ...
  const handleSubmit = () => {
        ...
        return;
          ...
          if (props.data) {
            ...
            promise = parseServerActionResult(
              createCaretakerWithProperties(caretakerData, selectedPropertyIds)
            );
          }

          runPromise(promise, "Saving Caretaker data...");
        });
      };
      void run();
      return;
    ...
    startTransition(() => {
      ...
      if (props.data && mode === "properties") {
        promise = parseServerActionResult(
          updateCaretakerProperties(props.data.id, selectedPropertyIds)
        );
      } else {
        if (!validateAll()) {
          releaseSubmitLock();
        ...
        <div className="grid w-full grid-cols-2 gap-5">
          <div>
            <div className="mb-2 block">
              <Label htmlFor="firstName">Caretaker First Name <span className="text-red-500">*</span></Label>
            </div>
            <TextInput
              id="firstName"
              name="firstName"
          ...
          <div>
            <div className="mb-2 block">
              <Label htmlFor="lastName">Caretaker Last Name</Label>
            </div>
            <TextInput
              id="lastName"
              name="lastName"
          ...
          <div>
            <div className="mb-2 block">
              <Label htmlFor="email">Caretaker Email <span className="text-red-500">*</span></Label>
            </div>
            <TextInput
              id="email"
              name="email"
          ...
          <div>
            <div className="mb-2 block">
              <Label htmlFor="mobileNumber">Caretaker Mobile Number <span className="text-red-500">*</span></Label>
            </div>
            <TextInput
              id="mobileNumber"
              name="mobileNumber"
          ...
          <div>
            <div className="mb-2 block">
              <Label htmlFor="whatsappNumber">Caretaker Whatsapp Number</Label>
            </div>
            <TextInput
              id="whatsappNumber"
              name="whatsappNumber"

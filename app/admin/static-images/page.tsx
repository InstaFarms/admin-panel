...
import { BREADCRUMBS, getSectionLabel, SECTIONS, TABLE_COLUMNS } from "@/constants/staticImages";
...
import DeleteStaticImageButton from "./DeleteStaticImageButton";
...
export default async function Page({ searchParams }: ServerPageProps) {
  ...
  try {
    const rawData = (await getAllStaticImages()) as StaticImageListItem[];
    ...
    <div className="flex w-full flex-col">
      <Card className="w-full bg-white dark:bg-gray-800">
        <div className="space-y-6">
          <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-gradient-to-br from-slate-950 via-slate-900 to-sky-950 p-6 text-white shadow-xl dark:border-slate-700">
            <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
              <div className="max-w-3xl">
                ...
                <h1 className="mt-4 text-3xl font-semibold tracking-tight">Static Website Images</h1>
                <p className="mt-2 max-w-2xl text-sm text-slate-300">
                  Manage reusable website artwork by section, with quick access to previews, ordering, and publishing status.
                </p>
                <PageBreadcrumb items={BREADCRUMBS.list} className="mt-4 text-slate-200" />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                ...
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  <Link href="/admin/static-images/create" className="inline-flex h-full w-full items-center justify-center rounded-xl bg-white/10 px-3 py-3 text-sm font-semibold text-white transition hover:bg-white/20">
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900/70">
            <div className="mb-5 flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400">
                  Jump straight into the area you want to update.
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {SECTIONS.map((sec) => (
                <Link key={sec.value} href={`/admin/static-images/${sec.value}`} className="group block">
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900/70">
            <div className="mb-5 flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400">
                  Review the latest assets across every section.
                </p>
              </div>
            </div>

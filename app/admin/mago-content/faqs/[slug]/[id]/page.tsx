...
export default async function Page({ params }: ServerPageProps) {
  ...
  const data = await getFaqById(idString, SCOPE);
    ...
    <div className="flex w-full flex-col">
      <Card className="w-full bg-white dark:bg-gray-800">
        <div className="flex flex-col gap-2">
          ...
          <PageBreadcrumb items={getFAQEditBreadcrumbs(category, SCOPE)} />
        </div>
        ...
        <div className="mx-auto flex w-[900px] flex-col gap-5 overflow-x-auto rounded-xl bg-slate-100 p-5 dark:bg-gray-900">
          ...
          <FAQEditor data={data} category={category} brandScope={SCOPE} />
        </div>
      </Card>
    </div>
  );

...
import { FAQ_CONTENT_BASE } from "@/constants/brandAdminScope";
import {
  Button,
  Card,
  Table,
...
const FAQ_BASE = FAQ_CONTENT_BASE[SCOPE];
...
export default async function Page({ searchParams, params }: ServerPageProps) {
  ...
  const breadcrumbs = getFAQListBreadcrumbs(category, SCOPE);
  ...
  try {
    const result = await getFaqs(limit, offset, category, searchValue, SCOPE);
    data = result?.data ?? [];
  } catch (err) {

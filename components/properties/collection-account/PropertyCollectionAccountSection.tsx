 ...
 * receives its manual guest payments. Owner collection stays disabled until
 * an owner is assigned; Mago (PLATFORM) collection is always available.
 */
export default function PropertyCollectionAccountSection({
  ...
  return (
    <div className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-900">
      <div>
        <h6 className="text-lg font-bold tracking-tight text-gray-900 dark:text-white">
          Property collection account
        </h6>
        <p className="text-sm text-gray-600 dark:text-gray-300">
          Choose who receives future manual guest payments for this property. This is separate from the owner&apos;s withdrawal account.
        </p>
      </div>

      {loading && !data ? (
        <p className="text-sm text-gray-500 dark:text-gray-400">Loading collection account…</p>
      ) : data ? (
        <>
          ...
          {!addNew ? (
            <div>
              <Label htmlFor="propertyCollectionAccount">Saved {receiverType === "PLATFORM" ? "Mago" : "owner"} account</Label>
              <Select
                id="propertyCollectionAccount"
                value={accountId}
            ...
            <div className="grid gap-3 rounded-lg border border-slate-200 p-4 md:grid-cols-2">
              {receiverType === "PLATFORM" ? (
                <div className="md:col-span-2">
                  <Label htmlFor="propertyCollectionLabel">Account label</Label>
                  <TextInput id="propertyCollectionLabel" value={newAccount.label || ""} onChange={(event) => updateNewAccount("label", event.target.value)} placeholder="e.g. Mago HDFC Collections" />
                </div>
              ) : null}
              <div>
                <Label htmlFor="propertyCollectionHolder">Account holder</Label>
                <TextInput id="propertyCollectionHolder" value={newAccount.accountHolderName} onChange={(event) => updateNewAccount("accountHolderName", event.target.value)} />
              </div>
              <div>
                <Label htmlFor="propertyCollectionNumber">Account number</Label>
                <TextInput id="propertyCollectionNumber" inputMode="numeric" value={newAccount.accountNumber} onChange={(event) => updateNewAccount("accountNumber", event.target.value)} />
              </div>
              <div>
                <Label htmlFor="propertyCollectionIfsc">IFSC</Label>
                <TextInput id="propertyCollectionIfsc" value={newAccount.ifscCode} onChange={(event) => updateNewAccount("ifscCode", event.target.value.toUpperCase())} />
              </div>
              <div>
                <Label htmlFor="propertyCollectionBank">Bank name</Label>
                <TextInput id="propertyCollectionBank" value={newAccount.bankName} onChange={(event) => updateNewAccount("bankName", event.target.value)} />
              </div>
              <div className="md:col-span-2">
                <Label htmlFor="propertyCollectionBranch">Branch (optional)</Label>
                <TextInput id="propertyCollectionBranch" value={newAccount.branch || ""} onChange={(event) => updateNewAccount("branch", event.target.value)} />
              </div>
            </div>
          ) : null}
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">Could not load the collection account for this property.</p>
      )}
    </div>
  );
}

...
interface AddActivitySearchBarProps {
  options: Activity[];
  addedIds: string[];
...
export default function AddActivitySearchBar({
  options,
  addedIds,
  onSelect,
  onInteract,
  ...
  return (
    <div className="relative my-3 w-full max-w-xl" ref={dropdownRef}>
      ...
      <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
        {!isLoaded
          ? "Focus, click, or type to load activities."
          : isLoading
          ? "Loading activities..."
          : `${availableOptions.length} activities available to add.`}
      </p>
      {showResults && (

...
import { GBVTrendChartData } from "@/utils/types";

import { formatINR } from "@/lib/dashboardUtils";

import GBVChartTooltip from "@/components/dashboard/GBVChartTooltip";

import { SkeletonChart } from "@/components/ui/Skeleton";

interface GBVTrendChartProps {
    data: GBVTrendChartData[];
    loading: boolean;
    error: boolean;
}

export default function GBVTrendChart({ data, loading, error }: GBVTrendChartProps) {
    ...
    if (error) {
        return (
            <div className="h-[400px] w-full flex items-center justify-center bg-red-50 dark:bg-red-900/10 rounded-xl border border-red-100 dark:border-red-900/30">
                <div className="text-red-500">Failed to load GBV data</div>
            </div>
        );
    }
    ...
    if (!data || data.length === 0) {
        return (
            <div className="h-[400px] w-full flex items-center justify-center bg-gray-50 dark:bg-gray-900/50 rounded-xl border border-dashed border-gray-300 dark:border-gray-700">
                <div className="text-gray-400">No GBV data available for selected range</div>
            </div>
        );
    }

    // Compute max value for the weekend background bar height
    const maxValue = Math.max(...data.map(d => Math.max(d.dailyGBV, d.movingAverage)), 1);

    // Weekend background data
    const chartData = data.map(d => ({
        ...d,
        weekendBg: d.isWeekend ? maxValue * 1.2 : 0,
    }));
    ...
    return (
        <div ref={cardRef} className="h-[400px] w-full bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700">
            <div className="mb-4 flex items-center justify-between">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white">GBV Trends</h3>
                <div className="flex items-center gap-4 text-xs">
                    ...
                    <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-gray-500 dark:text-gray-500">Daily GBV</span>
                    </div>
                    <div className="flex items-center gap-1.5">
            ...
            <ResponsiveContainer width="100%" height="100%" minHeight={320}>
                <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 50 }}>
                    ...
                    <Tooltip content={<GBVChartTooltip />} cursor={{ stroke: '#6b7280', strokeWidth: 1, strokeDasharray: '4 4' }} />

                    {/* Layer 1: The Ghost Line (Daily GBV) */}
                    <Line
                        type="monotone"
                        dataKey="dailyGBV"
                        stroke="#9ca3af"
